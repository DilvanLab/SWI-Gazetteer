...
public class DBpedia {
	...
	public ArrayList<Place> pull_query() throws NumberFormatException, FileNotFoundException, IOException{
		...
		query="PREFIX rdf:<http://www.w3.org/1999/02/22-rdf-syntax-ns#>  "
    			...
    			+ " ?place geo:long ?long . }";
		query(query, dbpedia_places,poly);
		Star_algorithm.fLogger.log(Level.SEVERE,query, dbpedia_places.size());
	...
	public void query(String query, ArrayList<Place> dbpedia_places, Polygon poly) throws NumberFormatException, FileNotFoundException, IOException{
		...
		Star_algorithm.fLogger.log(Level.SEVERE,query);
		String service="http://dbpedia.org/sparql";
		QueryExecution qe=QueryExecutionFactory.sparqlService(service, query);
		...
		Star_algorithm.fLogger.log(Level.SEVERE,"Search done!");
	}
	...
	public static OntModel OpenConnectOWL(){
		...
		if(in == null){
			Star_algorithm.fLogger.log(Level.SEVERE,"ERRO AO CARREGAR A ONTOLOGIA");
		}
	...
	public List<County> getMunicipalityFromAmazonas(ArrayList<Place> places) throws Exception{
