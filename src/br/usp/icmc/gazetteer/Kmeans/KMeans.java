...
public class KMeans {
	 ...
	 */
	public KMeans( ArrayList<Place> pointsFile, int k) {
		if (k < 2)
			new Exception("The value of k should be 2 or more.").printStackTrace();
		this.k = k;
		this.allPoints = pointsFile;
	}
	...
	private void getInitialKRandomSeeds(){
		...
		List<Place> kRandomPoints = getKRandomPoints();
		for (int i = 0; i < k; i++){
	...
	private List<Place> getKRandomPoints() {
		List<Place> kRandomPoints = new ArrayList<Place>();
		...
		for (int i = 0; i < k; i++) {
			...
			for (int j = 0; j < r; j++) {
				index++;
				while (index<allPoints.size() && alreadyChosen[index])
					index++;
			}
	...
	public static boolean agroup(FileWriter writer,ArrayList<Place> all_place,ArrayList<Group> group1) throws IOException{
		...
		if(kvalue<2)
			kvalue=2;
		if(all_place.size()>2){
			KMeans kMeans = new KMeans(all_place,kvalue);
			List<Cluster> pointsClusters = new ArrayList<Cluster>();
